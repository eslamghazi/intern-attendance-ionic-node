    ...
    for (const file of controllerFiles(SRC)) {
        ...
        for (const exported of Object.values(mod)) {
            ...
            for (const handler of Object.getOwnPropertyNames(cls.prototype)) {
                ...
                const hasPage = Reflect.getMetadata(PAGE_KEY, fn) ?? Reflect.getMetadata(PAGE_KEY, cls);
                const anyStaff = Reflect.getMetadata(ANY_STAFF_KEY, fn) ?? Reflect.getMetadata(ANY_STAFF_KEY, cls);
                routes.push({
...
describe('route authorization is always declared', () => {
    ...
    it('every route an admin may call names its page, or says any admin will do', () => {
        ...
        const unmapped = routes
            ...
            .map((r) => `${r.handler}  (${r.file})`);
        expect(unmapped, `These routes admit admins but say nothing about which page grant they ` +
