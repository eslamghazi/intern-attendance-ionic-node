...
function drizzleWrapped(pgError) {
    const err = new Error('Failed query: insert into public.attachments …');
    err.name = 'DrizzleQueryError';
    err.cause = pgError;
    return err;
}
const pgError = (code, message, extra = {}) => Object.assign(new Error(message), { code, ...extra });
describe('toApiError', () => {
    ...
    it('unwraps a Postgres error drizzle wrapped', () => {
        ...
        const api = toApiError(drizzleWrapped(pgError('42501', 'new row violates row-level security policy')));
    ...
    it('keeps the raw pg fields the client reads', () => {
        ...
        const api = toApiError(drizzleWrapped(pgError('23503', 'update or delete violated a constraint', {
