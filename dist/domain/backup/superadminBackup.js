 ...
 * "invalid input".
 */
export function parseSuperadminBackup(value) {
    ...
    if (root.kind !== SUPERADMIN_BACKUP_KIND) {
        return bad(`this is not a superadmin backup (expected kind "${SUPERADMIN_BACKUP_KIND}", got ` +
            `${typeof root.kind === 'string' ? `"${root.kind}"` : 'nothing'})`);
    }
    if (typeof root.version !== 'number' || root.version > SUPERADMIN_BACKUP_VERSION) {
        return bad(`this backup is version ${String(root.version)}, and this server understands up to ` +
            `${SUPERADMIN_BACKUP_VERSION} — use a newer server to restore it`);
    }
    if (!Array.isArray(root.accounts))
        return bad('the file carries no accounts list');
    if (!root.accounts.length)
        return bad('the backup is empty — there is nothing to restore');
    const accounts = [];
    for (const [i, raw] of root.accounts.entries()) {
        ...
        if (!nationalId)
            return bad(`${at} has no national_id`);
        if (!fullName)
            return bad(`${at} (${nationalId}) has no full_name`);
        ...
        if (!passwordHash)
            return bad(`${at} (${nationalId}) has no password_hash`);
        accounts.push({
            nationalId,
            fullName,
            passwordHash,
...
//# sourceMappingURL=superadminBackup.js.map