...
export function parsePanels(raw) {
    if (!raw)
        return [];
    const known = new Set(DASHBOARD_PANELS);
    const out = [];
...
function statusItems(s, t, withPending) {
    const items = [
...
function perSeries(list, values, idOf, color) {
    return list
        .map((item) => ({
        label: item.name,
...
function dailyRate(s, color) {
    return s.perDay.map((d) => d.settled > 0
        ? { label: String(d.day), value: d.rate, color }
        : { label: String(d.day), value: 0, color, gap: true });
}
export function dashboardCharts(panels, input) {
