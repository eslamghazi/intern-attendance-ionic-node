...
import { daysInMonth, monthRate, representativeStatus, statusFill, statusMark, } from './matrix.js';
describe('representativeStatus', () => {
    ...
    it('orders the whole severity chain', () => {
        const worstFirst = ['absent', 'late', 'left_work', 'early_leave', 'pending', 'present'];
...
describe('statusFill / statusMark', () => {
    it('gives every status a fill and a mark', () => {
        const all = ['present', 'late', 'early_leave', 'absent', 'left_work', 'pending'];
        for (const s of all) {
