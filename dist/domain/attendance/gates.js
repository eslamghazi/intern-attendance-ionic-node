...
import { CheckinMethod, AuditEvent, EnrollmentStatus, AttendanceRefusalReason, } from '../../common/enums/index.js';
/** Any TRUE across global, member, branch and group turns a bypass on. */
function anyOn(global, member, branch, group) {
 ...
 */
export function resolveBypass({ settings, member, now, qrAccepted }) {
    ...
    const face = anyOn(settings.bypassFace, member.bypassFace, branch?.bypassFace, group?.bypassFace);
    let location = anyOn(settings.bypassLocation, member.bypassLocation, branch?.bypassLocation, group?.bypassLocation);
    let source = location ? 'flag' : null;
    ...
    return {
        ...
        checkoutWindow: anyOn(settings.bypassCheckoutWindow, member.bypassCheckoutWindow, branch?.bypassCheckoutWindow, group?.bypassCheckoutWindow),
        blocked,
        requireQr,
        qrEnabled,
    };
...
export function bypassSnapshot(b) {
    if (!b.face && !b.location && !b.shiftWindow)
        return null;
 ...
 */
export function integrityOk(token, required) {
 ...
 */
export function checkGates(payload, settings, bypass, member, geofence) {
    const fail = (refusal, event, detail = null) => ({
        refusal,
        distance: 0,
        audit: event ? { event, detail } : null,
    });
    if (bypass.blocked)
        return fail(refuse(403, AttendanceRefusalReason.BRANCH_BLOCKED));
    ...
    if (!bypass.location) {
        ...
        if (!geofence)
            return fail(refuse(500, AttendanceRefusalReason.GEOFENCE_ERROR));
