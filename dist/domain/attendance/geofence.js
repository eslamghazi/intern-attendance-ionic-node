...
import { BOUNDARY_TOLERANCE_M as TOLERANCE_M, DEG, WGS84_A, WGS84_E2, } from '../../config/constants.js';
 ...
 */
function metresPerRadian(latDeg) {
...
function project(p, origin, scale) {
    return {
 ...
 */
export function readRing(areaCoords) {
    if (!Array.isArray(areaCoords) || areaCoords.length < 3)
        return null;
    const ring = [];
 ...
 */
export function evaluateGeofence(branch, fix) {
    const origin = { lat: branch.latitude, lng: branch.longitude };
