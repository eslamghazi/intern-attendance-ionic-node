...
import { decided, refuse, rejected, } from './types.js';
import { AttendanceStatus, CheckoutStatus, AttendanceRefusalReason, AuditEvent, } from '../../common/enums/index.js';
/**
 ...
 */
export function decideCheckIn(rostered, todayAttendance, ctx) {
    if (!rostered.length)
        return rejected(refuse(422, 'not_rostered'));
    const checkedIn = new Set(todayAttendance.filter((a) => a.checkInAt).map((a) => a.shiftId));
    const candidates = rostered.filter((s) => !checkedIn.has(s.id));
    if (!candidates.length)
        return rejected(refuse(409, AttendanceRefusalReason.ALREADY_CHECKED_IN));
    const win = (s) => placeWindow(s, ctx.minutesOfDay, ctx.defaults);
    let shift;
 ...
 */
export function decideCheckOut(todayOpen, yesterdayOpen, ctx, bypass) {
    const overnightOnly = yesterdayOpen.filter((o) => o.record.shift && isOvernight(o.record.shift));
    const open = [...todayOpen, ...overnightOnly];
    if (!open.length)
        return rejected(refuse(409, AttendanceRefusalReason.NOT_CHECKED_IN));
    const win = (s) => placeWindow(s, ctx.minutesOfDay, ctx.defaults);
    let chosen;
    if (ctx.settings.enforceShiftWindow && !bypass.checkoutWindow) {
        ...
        open.sort((a, b) => (a.record.checkInAt ?? '').localeCompare(b.record.checkInAt ?? ''));
        chosen = open[0];
    }
