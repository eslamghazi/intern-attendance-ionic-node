...
export function opsOnPage(page) {
    return PAGE_OPS[page];
}
/** The pages an admin may open. */
export function grantedPages(grant) {
    return new Set(grant?.pages ?? []);
}
/**
 ...
 */
export function grantedOps(grant, page) {
    if (!grantedPages(grant).has(page))
        return new Set();
...
const asList = (one) => (Array.isArray(one) ? one : [one]);
/**
 ...
 */
export function grantAllows(grant, pages, op) {
