...
import { folderOwner, mayTouchAttachment, pathOwner } from './attachment.js';
import { Role } from '../../common/enums/index.js';
const A = 'aaaaaaaa-0000-0000-0000-00000000000a';
const B = 'bbbbbbbb-0000-0000-0000-00000000000b';
const may = (kind, path, action, callerId, role = Role.MEMBER) => mayTouchAttachment({ kind, path, action, callerId, role });
/**
 ...
 */
const owns = (kind, path, action, callerId, subjectId, role = Role.MEMBER) => mayTouchAttachment({ kind, path, action, callerId, role, subjectId });
describe('pathOwner', () => {
...
describe('faces and probes', () => {
    for (const kind of ['faces', 'probes']) {
...
describe('the recorded subject decides, not the path', () => {
    ...
    const withSubject = (kind, path, action, callerId, subjectId, role = Role.MEMBER) => mayTouchAttachment({ kind, path, action, callerId, role, subjectId });
...
describe('an upload with no subject on record', () => {
    ...
    const upload = (path, callerId) => mayTouchAttachment({ kind: 'faces', path, action: 'write', callerId, role: Role.MEMBER });
    ...
    it('refuses probes the same way', () => {
        const probe = (path, callerId) => mayTouchAttachment({ kind: 'probes', path, action: 'write', callerId, role: Role.MEMBER });
