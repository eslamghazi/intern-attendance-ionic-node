...
const line = { ...columns, kind: 'line', title: 'خط' };
const stacked = {
    kind: 'stacked',
...
const gauge = { kind: 'gauge', title: 'مؤشر', percent: 76, label: 'نسبة الحضور', color: '#0d9488' };
...
describe('renderChartSvg', () => {
    ...
    it('a lone full slice is a whole ring, not a degenerate arc', () => {
        const only = { ...donut, items: [{ label: 'حضور', value: 5, color: '#16a34a' }] };
...
describe('the workbook', () => {
    ...
    it('adds a charts sheet after the table, drawn in cells', async () => {
        ...
        const filled = (row) => {
            let n = 0;
            for (let c = 3; c < 40; c++)
                if (ws.getCell(row, c).fill?.type === 'pattern')
                    n++;
            return n;
        };
    ...
    it('carries the copyright line in the footer', async () => {
        ...
        for (let r = 1; r <= ws.rowCount; r++)
            values.push(ws.getCell(r, 1).value);
