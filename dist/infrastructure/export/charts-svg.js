 ...
 * not a blob.
 */
function bar(x, y, length, thick, dir, color) {
...
function legend(items, y, rtl) {
    ...
    return items
        ...
        return (`<rect x="${sw}" y="${y - 6}" width="12" height="12" rx="3" fill="${it.color}"/>` +
 ...
 */
function frame(height, body, title, rtl, half = false) {
...
function donut(c, rtl) {
    ...
    if (c.centerTop)
        body += text(cx, cy - 8, c.centerTop, { anchor: 'middle', size: 24, weight: 700 });
    if (c.centerBottom)
        body += text(cx, cy + 16, c.centerBottom, { anchor: 'middle', size: 11, color: MUTED });
...
function gauge(c, rtl) {
    ...
    const arc = (from, to, color, opacity = 1) => {
...
function dailyFrame(items, yMax, ySuffix, rtl) {
    const top = 14;
    const bottom = 172;
    const left = rtl ? 8 : 40;
    ...
    const xOf = (i) => (rtl ? W - right - (i + 0.5) * slot : left + (i + 0.5) * slot);
    let body = '';
...
function heatmap(c, rtl) {
    const cell = 30;
    const gap = 3;
    const cols = 7;
    const left = (W - cols * (cell + gap)) / 2;
    const top = 22;
    const colX = (weekday) => (rtl ? left + (cols - 1 - weekday) * (cell + gap) : left + weekday * (cell + gap));
    let body = '';
...
export function renderChartSvg(chart, rtl) {
    switch (chart.kind) {
        case 'donut':
...
export function renderChartsSvg(charts, rtl) {
    if (!charts?.length)
        return '';
    return `<section class="charts">${charts.map((c) => renderChartSvg(c, rtl)).join('')}</section>`;
}
//# sourceMappingURL=charts-svg.js.map