...
function title(ws, at, text, font) {
    at.row += 1;
...
function barRow(ws, at, item, max, font) {
    const r = at.row;
    ...
    for (let i = 0; i < len; i++)
        fill(ws.getCell(r, FIRST_BAR_COL + i), item.color);
    at.row += 1;
}
/** One stacked row: label | total | ██▓▓░ segments in proportion. */
function stackRow(ws, at, label, segments, max, font) {
    const r = at.row;
    ...
    for (const seg of segments) {
        ...
        for (let i = 0; i < len; i++)
            fill(ws.getCell(r, col + i), seg.color);
        col += len;
    }
    at.row += 1;
}
function legendRow(ws, at, items, font) {
    const r = at.row;
    let col = 1;
...
function heatmapBlock(ws, at, chart, font) {
    ...
    for (const d of chart.cells) {
        ...
        if (d.value !== null)
            fill(cell, blend(chart.color, 0.16 + 0.84 * (d.value / 100)));
        slot++;
    }
    const weeks = Math.ceil((chart.firstWeekday + chart.cells.length) / 7);
    for (let w = 0; w < weeks; w++)
        ws.getRow(at.row + w).height = 28;
    at.row += weeks;
}
/** Add a "charts" sheet to the workbook. No-op without charts. */
export function addChartsSheet(wb, charts, opts) {
    if (!charts?.length)
        return;
    ...
    for (let i = 0; i < BAR_CELLS; i++)
        ws.getColumn(FIRST_BAR_COL + i).width = 1.4;
    const at = { row: 0 };
        ...
        switch (chart.kind) {
            ...
            case 'stacked': {
                ...
                for (const row of chart.rows)
                    stackRow(ws, at, row.label, row.segments, max, opts.font);
...
//# sourceMappingURL=charts-sheet.js.map