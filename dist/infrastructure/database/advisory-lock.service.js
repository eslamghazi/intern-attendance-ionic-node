...
let AdvisoryLockService = class AdvisoryLockService {
    ...
    async withLock(name, work) {
        ...
        try {
            const held = await client.query('select pg_try_advisory_lock(hashtext($1)::bigint) as locked', [key]);
     ...
     */
    async withLockBlocking(name, work) {
        const key = `job:${name}`;
        const client = await this.pool.connect();
        try {
...
AdvisoryLockService = __decorate([
    Injectable(),
    __param(0, Inject(PG_POOL)),
    __metadata("design:paramtypes", [Object])
], AdvisoryLockService);
export { AdvisoryLockService };
