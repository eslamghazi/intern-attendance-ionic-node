...
let SchedulerService = class SchedulerService {
    ...
    running = new Set();
    stopped = false;
    constructor(uow, locks, audit, storage) {
        this.uow = uow;
        this.locks = locks;
        this.audit = audit;
                ...
                run: async () => {
                    ...
                    const n = await this.uow.transaction(async (db) => {
                        ...
                        const open = await db
                            ...
                            .where(and(isNotNull(attendance.checkInAt), isNull(attendance.checkOutAt), 
                        // `is distinct from 'left_work'` — a null status counts as not
                        // yet marked, which a plain `<>` would silently exclude.
                        or(isNull(attendance.checkoutStatus), ne(attendance.checkoutStatus, CheckoutStatus.LEFT_WORK)), gte(attendance.date, previousDate(now.date))));
                        const overdue = open
                            .filter((row) => {
     ...
     */
    async tick(job) {
        if (this.stopped || this.running.has(job.name))
            return;
        ...
        try {
            ...
            if (note)
                console.log(`[scheduler] ${job.name}: ${note} (${Date.now() - started}ms)`);
        }
        catch (err) {
