...
describe('sign / verify', () => {
    it('accepts a token it just issued', () => {
        const { expires, signature } = sign(KIND, PATH, 60);
        expect(verify(KIND, PATH, expires, signature)).toBe('ok');
    });
    it('refuses a token issued for a DIFFERENT path', () => {
        // The signature covers the path; without that, one token would open every
        // file in the kind.
        const { expires, signature } = sign(KIND, PATH, 60);
        expect(verify(KIND, '2026/2026010108.jpg', expires, signature)).toBe('invalid');
    });
    it('refuses a token issued for a different kind', () => {
        const { expires, signature } = sign(KIND, PATH, 60);
        expect(verify('probes', PATH, expires, signature)).toBe('invalid');
    });
    it('refuses a token whose expiry was edited to extend it', () => {
        const { expires, signature } = sign(KIND, PATH, 60);
        expect(verify(KIND, PATH, expires + 86_400, signature)).toBe('invalid');
    });
    it('reports a genuinely expired token as expired, not invalid', () => {
        ...
        expect(verify(KIND, PATH, expires, signature)).toBe('expired');
    });
    ...
    it('refuses a missing, empty or garbage signature', () => {
        const { expires } = sign(KIND, PATH, 60);
        expect(verify(KIND, PATH, expires, '')).toBe('invalid');
        expect(verify(KIND, PATH, expires, 'not-a-signature')).toBe('invalid');
        expect(verify(KIND, PATH, expires, 'x'.repeat(43))).toBe('invalid');
    });
    it('refuses a non-numeric expiry', () => {
        const { signature } = sign(KIND, PATH, 60);
        expect(verify(KIND, PATH, Number.NaN, signature)).toBe('invalid');
    });
    it('cannot be confused by a delimiter shift between kind and path', () => {
    ...
    it('gives a different signature for every expiry', () => {
        vi.useFakeTimers();
        vi.setSystemTime(new Date('2026-08-29T09:00:00Z'));
        const first = sign(KIND, PATH, 60);
        vi.setSystemTime(new Date('2026-08-29T09:00:30Z'));
        const second = sign(KIND, PATH, 60);
        expect(second.signature).not.toBe(first.signature);
    });
});
describe('signedPath', () => {
    it('builds a URL that verifies', () => {
        const url = signedPath(KIND, PATH, 60);
        const q = new URLSearchParams(url.slice(url.indexOf('?') + 1));
        expect(url.startsWith(`/storage/${KIND}/object?`)).toBe(true);
        expect(verify(KIND, q.get('path'), Number(q.get('expires')), q.get('signature'))).toBe('ok');
    });
    it('percent-encodes a path with characters that would break the query', () => {
        const url = signedPath(KIND, 'a b/c&d.jpg', 60);
        const q = new URLSearchParams(url.slice(url.indexOf('?') + 1));
