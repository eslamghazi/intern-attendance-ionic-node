...
import { GetPresentQueryDto, GetReviewQueryDto, GetDetailQueryDto, GetHistoryQueryDto, GetDayQueryDto, GetDailyRosterQueryDto, GetMonthlyQueryDto, GetReportQueryDto, GetTodayQueryDto, GetStatsQueryDto, GetProbesDto, GetDashboardExportQueryDto, GetHistoryExportQueryDto, } from './dto/reports.dto.js';
...
let ReportsController = class ReportsController {
     ...
     */
    async exportHistory(caller, query, lang, reply) {
        if (!query?.member_id)
            throw badRequest('invalid_query', 'member_id is required');
        const entries = await this.reportsService.getHistory(caller, query.member_id, query.year, query.month);
        ...
        const tally = OUTCOME_LEGEND_ORDER.filter((k) => entries.some((e) => e.outcome === k))
            .map((k) => `${this.i18n.translate(ATTENDANCE_OUTCOME[k].labelKey, lang)}: ${entries.filter((e) => e.outcome === k).length}`)
            .join('  \u00b7  ');
    ...
    async getDailyRoster(caller, query) {
        if (!query?.date)
            throw badRequest('invalid_query', 'date is required');
     ...
     */
    async exportMonthly(caller, query, lang, reply) {
    ...
    async exportDashboard(caller, query, lang, reply) {
        ...
        const t = (key) => this.i18n.translate(`report.${key}`, lang);
        const nameOf = (list, id) => (id ? list.find((x) => x.id === id)?.name : undefined) ?? t('all');
        const pad = (n) => String(n).padStart(2, '0');
        ...
        const breakdown = (label, list, values, idOf) => {
            const named = list
                .map((item) => ({
                label: item.name,
...
__decorate([
    ...
    __param(3, Res()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, GetHistoryExportQueryDto, String, Object]),
    __metadata("design:returntype", Promise)
...
__decorate([
    ...
    __param(3, Res()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, GetMonthlyQueryDto, String, Object]),
    __metadata("design:returntype", Promise)
...
__decorate([
    ...
    __param(3, Res()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, GetDashboardExportQueryDto, String, Object]),
    __metadata("design:returntype", Promise)
...
__decorate([
    ...
    __param(0, Body()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [GetProbesDto]),
    __metadata("design:returntype", Promise)
