...
let ReportsService = class ReportsService {
    ...
    async getDetail(claims, memberId, date, shiftId) {
        return this.uow.asCaller(claims, async () => {
    ...
    async getHistory(claims, memberId, year, month) {
        return this.uow.asCaller(claims, async () => {
            if (!year || !month) {
    ...
    async getDay(claims, memberId, date) {
        return this.uow.asCaller(claims, async () => {
    ...
    async getDailyRoster(claims, date, branchId) {
        return this.uow.asCaller(claims, async () => {
    ...
    async getMonthly(claims, filters, year, month, page, pageSize) {
        const { first, last } = monthBounds(year, month);
    ...
    async getReport(claims, from, to, branchId, groupId) {
        return this.uow.asCaller(claims, async () => {
    ...
    async getStats(claims, year, month, f) {
        const { first, last } = monthBounds(year, month);
    ...
    async getProbes(claims, memberIds, from, to) {
        return this.uow.asCaller(claims, async () => {
