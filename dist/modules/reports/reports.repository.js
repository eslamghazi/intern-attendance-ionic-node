...
import { attendance, members, profiles, branches, groups, rosterDays, shifts, memberDepartments, memberDirectory, appSettings, } from '../../infrastructure/database/schema/index.js';
import { cairoNow } from '../../domain/clock.js';
import { slotConcluded } from '../../domain/attendance/slotConcluded.js';
import { eq, count, inArray, isNull, isNotNull, and, or, desc, asc, between, exists, sql, } from 'drizzle-orm';
import { dayInMonth, directoryWhere } from '../../domain/member/filter.js';
import { CheckType } from '../../common/enums/index.js';
function reachCondition(scope) {
    if (!scope)
        return undefined;
    const reach = [];
...
let ReportsRepository = class ReportsRepository extends GenericRepository {
    ...
    async slotContext() {
        ...
        return {
            ...
            clock: cairoNow(),
            byId: new Map(shiftRows.map((r) => [r.id, r])),
        };
    }
    ...
    async getDailyExpected(date, branchId, scope) {
    ...
    async getDailyAttendance(date, branchId, scope) {
        const conditions = [eq(attendance.date, date)];
        if (branchId)
            conditions.push(eq(attendance.branchId, branchId));
        const reach = reachCondition(scope);
    ...
    async getReportAttendanceBetween(from, to, branchId, groupId, scope) {
    ...
    async getStatsAttendance(first, last, f, scope) {
        ...
        if (f.day)
            conditions.push(eq(attendance.date, dayInMonth(first, f.day)));
        if (f.departmentId) {
    ...
    async getStatsRoster(first, last, f, scope) {
        ...
        if (f.day)
            conditions.push(eq(rosterDays.date, dayInMonth(first, f.day)));
        if (f.departmentId) {
            conditions.push(exists(this.db
                .select({ id: memberDepartments.id })
                .from(memberDepartments)
    ...
    async getProbesBetween(memberIds, from, to) {
        if (!memberIds.length)
            return [];
        const inRange = and(inArray(attendance.memberId, memberIds), between(attendance.date, from, to));
        const rows = await this.db
            .select({
            member_id: attendance.memberId,
            date: attendance.date,
            ...
            .from(attendance)
            .where(and(inRange, or(isNotNull(attendance.checkInProbePath), isNotNull(attendance.checkOutProbePath))));
