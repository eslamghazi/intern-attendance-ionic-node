...
let MembersService = class MembersService extends BaseService {
    ...
    async createMembers(caller, items) {
        ...
        const scope = await this.uow.transaction(async () => scopeOf(this.repo.db, caller));
        const results = [];
        for (const item of items) {
            ...
            try {
                results.push(await this.uow.transaction(async () => {
    ...
    async getMembers(filters, pageSize, offset) {
        return this.uow.transaction(async () => {
            const { rows, total } = await this.repo.getMembersDirectory(filters, pageSize, offset);
     ...
     */
    async lookup(caller, identifier) {
        return this.uow.transaction(async (tx) => {
            const row = await this.repo.findByIdentifier(identifier);
            if (!row)
                return { found: false };
            ...
            return { found: true, in_scope: inScope, member: row };
        });
    }
    async getMembersForExport(filters) {
        return this.uow.transaction(async () => this.repo.getMembersForExport(filters));
    }
    async getMembersPage(filters, pageSize, offset) {
        return this.uow.transaction(async () => {
            return this.repo.getMembersPage(filters, pageSize, offset);
        });
    }
    ...
    async updateMember(id, b) {
        return this.uow.transaction(async () => {
            ...
            if (Object.keys(memberSet).length)
                await repo.updateColumns('members', id, memberSet);
            return { ok: true };
        });
    }
     ...
     */
    async bulkUpdate(filters, patch) {
        const columns = MembersMapper.toMemberPatch(patch);
        if (!Object.keys(columns).length)
            return { affected: 0 };
        return this.uow.transaction(async () => {
            return this.repo.bulkUpdateMembers(filters, columns);
        });
    }
