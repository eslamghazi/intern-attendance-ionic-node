...
const MEMBER_COLUMN_CHECKED = MEMBER_COLUMN;
export class MembersMapper {
     ...
     */
    static toMemberPatch(input) {
        const patch = {};
        for (const field of MEMBER_FIELDS) {
            const value = input[field];
            if (value !== undefined)
                Object.assign(patch, { [MEMBER_COLUMN_CHECKED[field]]: value });
        }
        return patch;
    }
    /**
    ...
    static toProfilePatch(body) {
        ...
        if (body.avatar_url !== undefined)
            patch.avatarUrl = body.avatar_url;
        return patch;
    }
}
