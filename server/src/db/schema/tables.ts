import { pgTable, pgSchema, unique, uuid, text, timestamp, jsonb, foreignKey, boolean, bigint, index, uniqueIndex, vector, real, check, doublePrecision, integer, date, time, pgView, pgEnum } from "drizzle-orm/pg-core"
...
//
// The `auth` SCHEMA stays: auth.uid() and auth.jwt() read the request's JWT
// claims and are named by 14 policies. They are schema-qualified glue, not
// user storage.
