...
import type { AttachmentRequest } from './types.js';
export type { AttachmentRequest } from './types.js';

export type { FileAction, FileKind };
 ...
 * read as "everyone's".
 */
export function folderOwner(path: string): string | null {
...
function allows(rule: AccessRule, isOwn: boolean, staff: boolean): boolean {
  switch (rule) {
    case 'anyone':
      return true;
    case 'own':
 ...
 */
export function mayTouchAttachment(req: AttachmentRequest): boolean {
