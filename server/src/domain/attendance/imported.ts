...
/** The times as typed: 'HH:mm' in Cairo, or nothing. */
export interface ImportedTimes {
  checkIn: string | null;
  checkOut: string | null;
}

/** What gets written for one rostered slot. */
 ...
 * the morning after.
 */
export function importedSlot(
  shift: ShiftRow,
  date: string,
  times: ImportedTimes,
  settingsDefaults: WindowDefaults,
): ImportedSlot {
  ...
  if (outMin === null) return { status, checkoutStatus: null, checkInAt, checkOutAt: null };
