...
import { evaluateGeofence, readRing, type BranchGeofence, type LatLng } from './geofence.js';
...
const CENTRE: LatLng = { lat: BRANCH.latitude, lng: BRANCH.longitude };
...
function offset(north: number, east: number, from: LatLng = CENTRE): LatLng {
...
describe('projection accuracy', () => {
  ...
  it('is symmetric: A to B measures the same as B to A', () => {
    const b: LatLng = offset(300, 400);
    const there = evaluateGeofence({ ...BRANCH, radiusMeters: 5000 }, b);
    const back = evaluateGeofence(
      { latitude: b.lat, longitude: b.lng, radiusMeters: 5000, areaCoords: null },
      CENTRE,
    );
...
const SQUARE: [LatLng, LatLng, LatLng, LatLng] = [
  offset(-200, -200),
  offset(-200, 200),
  offset(200, 200),
  offset(200, -200),
...
describe('polygon geofence', () => {
  const polygon: BranchGeofence = { ...BRANCH, radiusMeters: 20, areaCoords: SQUARE };
