...
import { toMin, type ShiftRow } from './windows.js';
import {
  DEFAULT_CHECKIN_LEAD_MIN,
  DEFAULT_CHECKOUT_SPAN_MIN,
  MINUTES_PER_DAY,
} from '../../config/constants.js';
import type { SlotClock, SlotDefaults } from './types.js';
export type { SlotClock, SlotDefaults } from './types.js';
...
export function slotConcluded(args: {
  ...
  if (!shift || !enforceShiftWindow) return date < clock.date;
  ...
  if (clock.date !== endDate) return clock.date > endDate;
