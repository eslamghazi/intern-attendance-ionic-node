...
const bypassOf = (member = MEMBER, settings = SETTINGS, qrAccepted = false) =>
  resolveBypass({ settings, member, now: NOW, qrAccepted });
...
describe('resolveBypass', () => {
  ...
  it('turns a bypass on from ANY of the four levels', () => {
    ...
    expect(
      bypassOf({ ...MEMBER, branch: { ...MEMBER.branch!, bypassFace: true } }).face,
    ).toBe(true);
  ...
  it('records where a location bypass came from', () => {
    ...
    expect(
      bypassOf({ ...MEMBER, locationBypassUntil: '2026-08-29T09:30:00Z' }).source,
    ).toBe('window');
  ...
  it('takes the MOST RESTRICTIVE method across global and branch', () => {
    ...
    expect(
      bypassOf({ ...MEMBER, branch: { ...MEMBER.branch!, blockCheckin: true } }).blocked,
    ).toBe(true);
    expect(bypassOf(MEMBER, { ...SETTINGS, checkinMethod: 'qr' }).requireQr).toBe(true);
    expect(
      bypassOf({ ...MEMBER, branch: { ...MEMBER.branch!, requireQr: true } }).requireQr,
    ).toBe(true);
    expect(
      bypassOf({ ...MEMBER, branch: { ...MEMBER.branch!, qrEnabled: false } }).qrEnabled,
    ).toBe(false);
  });
});
...
describe('checkGates — refusals', () => {
  ...
  it('refuses a failed liveness check when liveness is required', () => {
    ...
    expect(
      gate({ livenessPassed: false }, MEMBER, { ...SETTINGS, livenessRequired: false }).refusal,
    ).toBeNull();
  });
...
describe('checkGates — what a bypass actually skips', () => {
  ...
  it('a location bypass skips mock GPS, accuracy AND the geofence together', () => {
    ...
    const r = checkGates(
      { ...PAYLOAD, isMock: true, accuracy: 9999 },
      SETTINGS,
      bypassOf(noLocation),
      noLocation,
      OUTSIDE,
  ...
  it('a QR-only branch refuses proximity, and accepts once QR cleared location', () => {
    ...
    const withQr = checkGates(
      PAYLOAD,
      SETTINGS,
      resolveBypass({ settings: SETTINGS, member, now: NOW, qrAccepted: true }),
      member,
      INSIDE,
    );
