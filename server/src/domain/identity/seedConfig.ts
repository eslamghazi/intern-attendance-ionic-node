...
import type { SeedVerdict } from './types.js';
export type { SeedVerdict } from './types.js';
 ...
 */
export function checkSuperadminConfig(nationalId: string, password: string): SeedVerdict {
  const refuse = (why: string): SeedVerdict => ({ kind: 'refuse', why });

  if (!nationalId && !password) return { kind: 'skip' };

  if (!nationalId) return refuse('SUPERADMIN_PASSWORD is set but SUPERADMIN_NATIONAL_ID is empty');
  if (!password) return refuse('SUPERADMIN_NATIONAL_ID is set but SUPERADMIN_PASSWORD is empty');
  ...
  if (/CHANGE_ME/i.test(password)) {
    return refuse(
      'SUPERADMIN_PASSWORD is still the placeholder from .env.example. ' +
  ...
  if (!/^\d{14}$/.test(nationalId)) {
    return refuse(
      `SUPERADMIN_NATIONAL_ID must be 14 digits (got ${nationalId.length} characters)`,
    );
  }
