...
import type { AdminPermissions } from './types.js';
...
describe('a page grant', () => {
  const grant: AdminPermissions = { pages: ['members', 'review'] };
...
describe('per-page operations', () => {
  ...
  it('cannot grant an operation the page does not have', () => {
    ...
    const odd: AdminPermissions = { pages: ['dashboard'], pageOps: { dashboard: ['delete', 'export'] } };
...
describe('the older page-wide list', () => {
  const grant: AdminPermissions = { pages: ['members', 'audit'], ops: ['edit', 'export'] };
...
describe('a route that accepts any of several', () => {
  ...
  it('is satisfied by either operation — an upsert is create or edit', () => {
    const creator: AdminPermissions = { pages: ['departments'], pageOps: { departments: ['create'] } };
    const editor: AdminPermissions = { pages: ['departments'], pageOps: { departments: ['edit'] } };
    const reader: AdminPermissions = { pages: ['departments'] };
