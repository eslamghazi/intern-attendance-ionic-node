...
export function classifyRefresh(
  ...
  if (!stored) return { kind: 'reject', reason: 'unknown' };
  ...
  if (stored.rotatedAt !== null) return { kind: 'reuse', familyId: stored.familyId };

  if (stored.revokedAt !== null) return { kind: 'reject', reason: 'revoked' };
  if (stored.expiresAt.getTime() <= now.getTime()) return { kind: 'reject', reason: 'expired' };
 ...
 * arithmetic on it.
 */
export function expiresInSeconds(ttlMinutes: number): number {
