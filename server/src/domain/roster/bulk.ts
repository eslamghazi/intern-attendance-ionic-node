 ...
 */
export function planBulk(mode: BulkMode, shiftId: string): BulkPlan {
  switch (mode) {
    case 'add':
      return {
...
export function rangeDates(year: number, month: number, fromDay: number, toDay: number): string[] {
  const pad = (n: number) => String(n).padStart(2, '0');
  const lastDay = new Date(year, month, 0).getDate();
  const from = Math.max(1, Math.min(fromDay, toDay));
  const to = Math.min(lastDay, Math.max(fromDay, toDay));
  const out: string[] = [];
  for (let d = from; d <= to; d++) out.push(`${year}-${pad(month)}-${pad(d)}`);
  return out;
}
