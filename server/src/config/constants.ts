...
export const FILE_KIND_NAMES = Object.keys(FILE_KINDS) as FileKind[];
...
} as const satisfies Record<FileKind, Record<FileAction, AccessRule>>;

export type FileAction = 'read' | 'write' | 'delete';
export type AccessRule = 'anyone' | 'own' | 'staff' | 'own-or-staff';
...
export type AttendanceOutcomeKey = keyof typeof ATTENDANCE_OUTCOME;

/** Legend order: the good outcomes, then the partial ones, then the failures. */
export const OUTCOME_LEGEND_ORDER: readonly AttendanceOutcomeKey[] = [
  'present_out',
  'present_open',
  'late_out',
  'late_open',
...
export const MEMBER_FIELDS = Object.keys(MEMBER_COLUMN) as (keyof typeof MEMBER_COLUMN)[];
