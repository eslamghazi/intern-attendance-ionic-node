...
import type { CheckInWrite, CheckOutWrite } from './attendance.types.js';
import type { JsonValue } from '../../common/json.types.js';
export type { CheckInWrite, CheckOutWrite } from './attendance.types.js';

@Injectable()
export class AttendanceRepository extends GenericRepository<typeof attendance> implements IAttendanceRepository {
  ...
  async redeemQrToken(args: { token: string; branchId: string; memberId: string; date: string; requiresMember: boolean }): Promise<boolean> {
    ...
    if (args.requiresMember) conditions.push(isNotNull(qrTokens.memberId));
   ...
   */
  async verifyFaceScore(memberId: string, embedding: readonly number[]): Promise<number | null> {
  ...
  async writeCheckIn(w: CheckInWrite): Promise<boolean> {
    const values = {
      ...
      branchId: w.branchId,
      date: w.date,
      status: w.status as typeof attendance.$inferInsert.status,
      shiftId: w.shiftId,
  ...
  async probeNaming(
    profileId: string,
  ): Promise<{ groupYear: number | null; code: string | null; branchName: string | null } | null> {
    const rows = await this.db
      .select({
  ...
  async writeAudit(actorId: string | null, event: string, detail: JsonValue): Promise<void> {
  ...
  async manualSetShift(memberId: string, date: string, shiftId: string | null) {
    ...
    if (!resolved) return { shift_id: null, shift_name: null };
