...
import type { CheckResult, ImportAttendanceResult, ImportAttendanceRow, ImportOutcome, SetAttendanceInput } from './attendance.types.js';
...
export class AttendanceService implements IAttendanceService {
   ...
   */
  async importAttendance(caller: Caller, rows: ImportAttendanceRow[]): Promise<ImportAttendanceResult> {
    return this.uow.transaction(async () => {
      ...
      const result: ImportAttendanceResult = { written: 0, no_roster: 0, not_yours: 0, invalid: 0, rows: [] };
      const memberCache = new Map<string, { branchId: string; groupId: string | null } | null>();

      for (const [index, row] of rows.entries()) {
        const outcome = async (): Promise<ImportOutcome> => {
          if (row.checkIn && parseClock(row.checkIn) === null) return 'invalid';
          if (row.checkOut && parseClock(row.checkOut) === null) return 'invalid';

          if (!memberCache.has(row.memberId)) memberCache.set(row.memberId, await this.repo.getMemberBranch(row.memberId));
          const member = memberCache.get(row.memberId);
          if (!member) return 'not_yours';
