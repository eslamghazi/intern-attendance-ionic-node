...
function columnsWritten(table: typeof members | typeof profiles, patch: object): string[] {
...
describe('toProfilePatch', () => {
  it('writes the name and the national id — the two that were being dropped', () => {
    expect(columnsWritten(profiles, MembersMapper.toProfilePatch(edit))).toEqual(
      expect.arrayContaining(['full_name', 'national_id']),
    );
  });
