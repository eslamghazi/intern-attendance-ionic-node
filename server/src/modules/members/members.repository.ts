...
import { eq, sql, inArray, count, and, like, isNotNull, or, type SQL } from 'drizzle-orm';
import {
  composeMemberCode,
  memberCodePrefix,
  memberCodeSerial,
  reissueNeeded,
} from '../../domain/member/code.js';
...
export type { DirectoryRow, ItemResult, MemberInput, MemberPageItem } from './members.types.js';

@Injectable()
export class MembersRepository extends GenericRepository<typeof members> implements IMembersRepository {
  constructor() {
  ...
  async updateColumns(
    ...
    id: string,
    patch: ProfilePatch | MemberPatch,
  ): Promise<void> {
  ...
  async upsertMember(callerId: string, input: MemberInput): Promise<ItemResult> {
    ...
    if (existing) {
      ...
      if (updated[0]) await this.ensureMemberCode(updated[0].id);
    ...
    if (inserted[0]) await this.ensureMemberCode(inserted[0].id);
   ...
   */
  private async cohortOf(groupId: string | null | undefined) {
   ...
   */
  private async nextSerial(prefix: string): Promise<number> {
    const rows = await this.db
   ...
   */
  async ensureMemberCode(memberId: string): Promise<string | null> {
    const rows = await this.db
    ...
    if (!reissueNeeded(current.code, cohort)) return current.code ?? null;
  ...
  async ensureMemberCodes(memberIds: readonly string[]): Promise<void> {
    for (const id of memberIds) await this.ensureMemberCode(id);
  }
   ...
   * and a directory.
   */
  async findByIdentifier(identifier: string): Promise<LookupRow | null> {
   ...
   */
  private async countDirectory(filters: MemberFilters): Promise<number> {
    const rows = await this.db
      .select({ n: count() })
      .from(memberDirectory)
      .where(directoryWhere(filters));
  ...
  async bulkUpdateMembers(filters: MemberFilters, patch: MemberPatch) {
