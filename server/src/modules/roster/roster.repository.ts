...
@Injectable()
export class RosterRepository extends GenericRepository<typeof rosterDays> implements IRosterRepository {
  constructor() {
   ...
   * window value.
   */
  async getRosterView(filters: any, year: number, month: number, first: string, last: string, pageSize: number | null, offset: number) {
  ...
  async getRosterTotals(filters: any, year: number, month: number, first: string, last: string) {
    ...
    for (const r of rows) {
      ...
      if (r.shift_id) (perDayShift[day] ??= {})[r.shift_id] = n;
      total += n;
    }
   ...
   */
  async getDepartmentNames(memberIds: string[], year: number, month: number) {
    ...
    const rows = await this.db
      ...
      .where(
        and(
          inArray(memberDepartments.memberId, memberIds),
  ...
  async getMakerSelf(callerId: string) {
  ...
  async getMakerMembers(branchId: string) {
  ...
  async getMakerRoster(branchId: string, first: string, last: string) {
    const rows = await this.db
   ...
   */
  async getExistingKeys(memberIds: string[], first: string, last: string) {
  ...
  async bulkOperation(o: any, first: string, last: string, dates: string[], plan: any) {
    ...
    const rows = await this.db.execute<{ members: string; added: string; removed: string }>(sql`
  ...
  async insertDays(insertValues: any[]) {
    const rows = await this.db
      .insert(rosterDays)
      .values(insertValues)
      .onConflictDoNothing()
  ...
  async deleteDay(memberId: string, date: string, shiftId: string) {
    await this.db
      .delete(rosterDays)
      .where(
        and(
