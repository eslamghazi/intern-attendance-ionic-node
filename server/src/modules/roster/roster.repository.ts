...
> {
  ...
  async getRosterTotals(filters: any, year: number, month: number, first: string, last: string) {
    const rows = await this.db.execute<{ day: number; shift_id: string | null; cnt: number }>(sql`
      with picked as (
  ...
  async getMakerRoster(branchId: string, first: string, last: string) {
    const rows = await this.db.execute<{ member_id: string; day: number; key: string | null }>(sql`
