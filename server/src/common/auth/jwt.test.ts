...
describe('what verifyToken refuses', () => {
  ...
  it('refuses a role that is not one of ours', () => {
    expect(
      verifyToken(forge({ sub: PROFILE, aud: TOKEN_AUDIENCE, user_role: 'wizard' })),
    ).toBeNull();
  });
  ...
  it('refuses a foreign signature', () => {
    const foreign = jwt.sign(
      { sub: PROFILE, aud: TOKEN_AUDIENCE, user_role: Role.SUPERADMIN },
      'not-the-secret-not-the-secret-32b',
  ...
  it('refuses an expired token', () => {
    const stale = jwt.sign(
      { sub: PROFILE, aud: TOKEN_AUDIENCE, user_role: Role.MEMBER },
      env.APP_JWT_SECRET,
  ...
  it('drops a national_id that is not a string rather than passing it on', () => {
    const claims = verifyToken(
      forge({ sub: PROFILE, aud: TOKEN_AUDIENCE, user_role: Role.MEMBER, national_id: 12345 }),
    );
    expect(claims).not.toBeNull();
