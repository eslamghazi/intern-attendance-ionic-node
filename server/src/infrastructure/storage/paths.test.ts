...
describe('facePath', () => {
  ...
  it('nests year / branch / group / member and ends in a fixed name', () => {
    expect(facePath(parts)).toBe(
      'year-2026/branch-el-mabarra-hospital/group-nursing-a/member-2026010001/face.jpg',
    );
  });
  ...
  it('names the gap instead of collapsing it, when something is missing', () => {
    expect(facePath({ groupYear: null, branchName: null, groupName: null, memberCode: null })).toBe(
      'year-no-group/branch-no-branch/group-no-group/member-unknown-member/face.jpg',
    );
  });
...
describe('probePath', () => {
  ...
  it('nests year / month / day / branch / member', () => {
    expect(probePath(parts)).toBe(
      'year-2026/2026-09/2026-09-11/branch-el-mabarra-hospital/member-2026010001/check-in__07-58__morning-shift.jpg',
    );
  });
  ...
  it('sorts by time within the day, because the time leads the file name', () => {
    ...
    const name = (p: string) => p.split('/').pop()!.replace(/^check-(in|out)__/, '');
...
describe('avatarPath', () => {
  it('stays flat and keyed by profile id', () => {
    expect(avatarPath('11111111-2222-3333-4444-555555555555')).toBe(
