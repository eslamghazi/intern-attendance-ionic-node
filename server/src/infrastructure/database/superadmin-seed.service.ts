...
import {
  BCRYPT_COST,
  FIRST_SUPERADMIN,
  FIRST_SUPERADMIN_FILE,
  FIRST_SUPERADMIN_PASSWORD_BYTES,
...
export class SuperadminSeedService {
  ...
  async ensure(): Promise<void> {
    await this.uow.transaction(async (tx) => {
      ...
      if (existing[0]) return; // somebody can already get in
  ...
  private announce(password: string): void {
    ...
    try {
      ...
      console.error(
        `[seed] could not write ${path} (${err instanceof Error ? err.message : String(err)}) — ` +
