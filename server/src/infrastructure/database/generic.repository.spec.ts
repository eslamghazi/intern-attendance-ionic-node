...
import type { Row } from './row.types.js';
...
type TestEntity = Row<typeof testTable>;

class TestRepository extends GenericRepository<typeof testTable> {
  constructor() {
...
function stubDb(rows: unknown[]) {
  ...
  vi.spyOn(dbContextStorage, 'getStore').mockReturnValue(db as never);
  return db;
}
...
describe('GenericRepository', () => {
  ...
  const row: TestEntity = { id: '123', name: 'Test' };
  ...
  it('create returns the inserted row', async () => {
    const created: TestEntity = { id: 'new-id', name: 'Created' };
    stubDb([created]);
  ...
  it('update returns the updated row', async () => {
    const updated: TestEntity = { id: '123', name: 'Updated' };
    stubDb([updated]);
